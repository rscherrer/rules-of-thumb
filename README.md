...
This section is about writing simulation code that is as efficient and reliable while still being simple enough for non-commercial, academic work in the field of ecology and evolution. More to come, but see [this page](https://github.com/rscherrer/need-for-speed) on how to make good use of a computer cluster.
...
Research guidelines --- a journey through trials and errors

Use R for analyses and plotting --> great plots, tons of packages

Organize research in analytical modules, where each analytical module answers a series of questions from a given set of data. For me, an analytical module will consist of a master script doing lots of things. This might be recommended against by other people, however, who might recommend instead to have several scripts. In any case, the key is to group all analyses (all scripts) relevant to one set of questions for one set of data into one module. One module therefore consists of scripts to run in order to replicate the results, given that the input data are provided. If the input data changes a bit, just re-run the scripts. Ideally the output should be automatically saved into a results section for that module. The results section can contain folders for different questions or different analyses, and can contain output files in the form of plots, tables or even R data sets (RDS). The idea is that the results section is a sink for all the outputs of the analysis, which we should not touch as they are automatically generated by the scripts, allowing anyone to simply check that the results are reproducible.

This means that when it comes to nice results and beautiful plots, we turn into another section: the manuscript section. I treat the results sections of my analytical modules like sources of interesting results and repository for back-up analyses. This means that I will pick from there one particularly representative simulation, or will combine the results of multiple tables into one table I want to show in my paper. The results in the results section need not be pretty: they need to be generated anew easily and can be used to show stuff to collaborators. They serve as raw materials for pretty content once copied and moved into the manuscript section. Keeping them as simple as possible also allows them to be used as input into the analytical modules!

Write the manuscript in LaTeX: great for formatting and allows a lot of modularity

Reviewing: use Word or Google Docs, possibly getting there from Latex using pandoc. Or, set up a special command in Latex to allow the supervisors to comment on Overleaf. This section is flexible as long as the collaborators can manage.

Organize manuscript in a "ms" folder with the various sections

Have a figure folder where we store what shows up in the manuscript (i.e. some figures from the results may need to be moved here)
...
Having figures and tables from the results and paper separated also allows to clear up space in the manuscript section easily: no need to crowd these folders with figures that we decided we did not want. We will still be able to find them in the results.

Golden rule: produce more results than you are going to need, because you will be asked to change stuff.
